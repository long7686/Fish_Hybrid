...
class gfFishGroup6 {
    //Total time: 38.5s
    static create() {
        const fishCount = [16,16,16,12,12,6,1,1,1,1];
        ...
        for (let i = 0; i < 3; ++i) {
            const count = fishCount[5] / 3;
            const xOffset = -i * 2 * space * count;
            FishGroupHelper.createFishLine(count, Y6, space, speed, xOffset);
        }

        //big fishes
        const bigSpace = space * fishCount[3] / 6;
        FishGroupHelper.createFishLine(1, Y6, 0, speed, -bigSpace * 1.35 );
        FishGroupHelper.createFishLine(1, Y6, 0, speed, -bigSpace * 3.15);
        FishGroupHelper.createFishLine(1, Y6, 0, speed, -bigSpace * 5.35);
        FishGroupHelper.createFishLine(1, Y6, 0, speed, -bigSpace * 7.15);
    }
}
