...
cc.Class({
    ...
    loadCustomManifest: function () {
        if (this._am.getState() === jsb.AssetsManager.State.UNINITED) {
            var manifest = new jsb.Manifest(customManifestStr, this._storagePath); // eslint-disable-line
    ...
    onLoad: function () {
        ...
        if (!cc.sys.isNative) {
            ...
            this.dateTime.node.active = false;
            return;
        }

        var versionText = this.versionFile.text.split('|');

        if (versionText.length >= 2)
        {
            versionText[0] = versionText[0].replace(/(\r\n|\n|\r)/gm, "");
            versionText[1] = versionText[1].replace(/(\r\n|\n|\r)/gm, "");
            versionText[2] = versionText[2].replace(/(\r\n|\n|\r)/gm, "");
            this.version.string = "Version: " + versionText[0];
            this.build.string = "Build: " + versionText[1];
            this.dateTime.string = "Date: " + versionText[2];
        }
